...
let candidates = [];
let mostPlayedList = [];   // ordered titles from popularity=mostplayed feed
let bestGamesList = [];    // ordered titles from popularity=bestgames feed
for (const f of files) {
  ...
  candidates = candidates.concat(games);
  const header = text.slice(0, 400); // first lines contain the fetched URL
  if (header.includes("popularity=mostplayed") && !mostPlayedList.length)
    mostPlayedList = games.map(g => g.title);
  if (header.includes("popularity=bestgames") && !bestGamesList.length)
    bestGamesList = games.map(g => g.title);
}
...
// ---- spotlight lists (ordered slugs, only games that exist in catalog) ----
const allSlugSet = new Set(
  [...curated.matchAll(/title:\s*"([^"]+)"/g)].map(m => slugify(m[1]))
    .concat(out.map(g => slugify(g.t)))
);
const toSlugs = (titles, n) => {
  const res = [];
  for (const t of titles) {
    const s = slugify(t.replace(/&amp;/g, "&").replace(/\s+/g, " ").trim());
    if (allSlugSet.has(s) && !res.includes(s)) res.push(s);
    if (res.length >= n) break;
  }
  return res;
};
const SPOTLIGHT = {
  reviewed: toSlugs(bestGamesList, 14),
  played:   toSlugs(mostPlayedList, 14)
};
console.log("spotlight reviewed:", SPOTLIGHT.reviewed.length, "| played:", SPOTLIGHT.played.length);
...
// Spotlight lists (ordered slugs): reviewed = GameMonetize "best games"
// ranking, played = GameMonetize "most played" ranking.
